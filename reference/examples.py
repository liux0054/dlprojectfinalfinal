...
master = pd.read_csv("data/train/train.csv")
...
for i in range(len(master)):
    file_paths.append(img_path + str(master.ix[i][0]))
...
for i in range(len(sample_submission)):
    ...
    file_paths.append(img_path + sample_submission.ix[i][0])
...
add_model.add(Dense(131, activation='softmax'))

model = Model(inputs=base_model.input, outputs=add_model(base_model.output))
model.compile(loss='mean_squared_error', optimizer=optimizers.SGD(lr=1e-4, momentum=0.9), metrics=['accuracy'])
